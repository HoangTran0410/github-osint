...
const EventListScreen: React.FC<EventListScreenProps> = ({ user, onSelectEvent, onSearch, isLoading }) => {
  ...
  const getEventIcon = (type: string) => {
    switch (type) {
      case 'PushEvent': return 'upload';
      case 'ForkEvent': return 'share';
      case 'WatchEvent': return 'star';
      case 'CreateEvent': return 'add';
      case 'PullRequestEvent': return 'rebase';
      case 'IssuesEvent': return 'adjust';
      case 'IssueCommentEvent': return 'chat_bubble';
      case 'DeleteEvent': return 'delete';
      case 'MemberEvent': return 'person_add';
      case 'PublicEvent': return 'public';
      case 'ReleaseEvent': return 'new_releases';
  ...
  const getEventDescription = (event: GitHubEvent, isReceived: boolean = false) => {
    ...
    const actorPrefix = isReceived ? (
      <span className="font-semibold text-slate-900 dark:text-white mr-1">
        <a href={`https://github.com/${event.actor.login}`} target="_blank" rel="noreferrer" className="hover:underline" onClick={(e) => e.stopPropagation()}>{event.actor.login}</a>
      </span>
    ) : null;
    ...
    switch (event.type) {
      case 'PushEvent':
        ...
        content = <>pushed to <span className="font-bold text-primary">{branch}</span> in <a href={`https://github.com/${event.repo.name}`} target="_blank" rel="noreferrer" className="font-bold text-primary hover:underline break-all" onClick={(e) => e.stopPropagation()}>{event.repo.name}</a></>;
        break;
      case 'ForkEvent':
        content = <>forked <a href={`https://github.com/${event.repo.name}`} target="_blank" rel="noreferrer" className="font-bold text-primary hover:underline break-all" onClick={(e) => e.stopPropagation()}>{event.repo.name}</a></>;
        break;
      case 'WatchEvent':
        content = <>starred <a href={`https://github.com/${event.repo.name}`} target="_blank" rel="noreferrer" className="font-bold text-primary hover:underline break-all" onClick={(e) => e.stopPropagation()}>{event.repo.name}</a></>;
        break;
      case 'CreateEvent':
        content = <>created repository <a href={`https://github.com/${event.repo.name}`} target="_blank" rel="noreferrer" className="font-bold text-primary hover:underline break-all" onClick={(e) => e.stopPropagation()}>{event.repo.name}</a></>;
        break;
      case 'PullRequestEvent':
        content = <>{event.payload.action} a pull request in <a href={`https://github.com/${event.repo.name}`} target="_blank" rel="noreferrer" className="font-bold text-primary hover:underline break-all" onClick={(e) => e.stopPropagation()}>{event.repo.name}</a></>;
        break;
      case 'IssuesEvent':
        content = <>{event.payload.action} issue in <a href={`https://github.com/${event.repo.name}`} target="_blank" rel="noreferrer" className="font-bold text-primary hover:underline break-all" onClick={(e) => e.stopPropagation()}>{event.repo.name}</a></>;
        break;
      case 'IssueCommentEvent':
        content = <>commented on issue in <a href={`https://github.com/${event.repo.name}`} target="_blank" rel="noreferrer" className="font-bold text-primary hover:underline break-all" onClick={(e) => e.stopPropagation()}>{event.repo.name}</a></>;
        break;
      default:
        content = <>{event.type.replace('Event', '')} on <a href={`https://github.com/${event.repo.name}`} target="_blank" rel="noreferrer" className="font-bold text-primary hover:underline break-all" onClick={(e) => e.stopPropagation()}>{event.repo.name}</a></>;
    }

    return <span>{actorPrefix} {content}</span>;
  };
  ...
  const renderRepoItem = (repo: GitHubRepo) => (
      <div className="flex flex-col gap-2 p-4 sm:p-5 rounded-xl border border-slate-200 dark:border-[#233648] bg-white dark:bg-[#192430] hover:border-primary/50 transition-colors">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-2">
              <a href={repo.html_url} target="_blank" rel="noreferrer" className="text-base font-bold text-primary hover:underline truncate pr-4">
                  {repo.full_name}
              </a>
              <div className="flex items-center gap-3 text-slate-500 dark:text-[#92adc9] text-sm shrink-0">
                  <span className="flex items-center gap-1" title="Stars"><span className="material-symbols-outlined text-sm">star</span> {repo.stargazers_count}</span>
                  <span className="flex items-center gap-1" title="Forks"><span className="material-symbols-outlined text-sm">call_split</span> {repo.forks_count}</span>
              </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-x-4 gap-y-2 mt-2 text-xs text-slate-500 dark:text-[#92adc9]">
              {repo.language && <span className="flex items-center gap-1"><span className="size-2 rounded-full bg-primary"></span> {repo.language}</span>}
              <span>Updated {moment(repo.updated_at).fromNow()}</span>
              
              <span className="flex items-center gap-1" title="Size"><span className="material-symbols-outlined text-sm">database</span> {formatSize(repo.size)}</span>
              <span className="flex items-center gap-1" title="Watchers"><span className="material-symbols-outlined text-sm">visibility</span> {repo.watchers_count}</span>
              
              {repo.archived && (
                  <span className="px-2 py-0.5 rounded-full bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 font-medium text-[10px] uppercase tracking-wider border border-amber-200 dark:border-amber-800">Archived</span>
              )}
          </div>
      </div>
  );
      ...
      <div className="flex flex-col gap-2 p-4 sm:p-5 rounded-xl border border-slate-200 dark:border-[#233648] bg-white dark:bg-[#192430] hover:border-primary/50 transition-colors">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-2">
              <div className="flex flex-col gap-1 min-w-0">
                 ...
                 <a href={gist.html_url} target="_blank" rel="noreferrer" className="text-base font-bold text-primary hover:underline truncate">
                     {Object.keys(gist.files)[0] || "Untitled Gist"}
                 </a>
              </div>
              <span className="text-xs text-slate-500 dark:text-[#92adc9] shrink-0">{moment(gist.created_at).format('MMM DD, YYYY')}</span>
          </div>
          ...
          <div className="flex flex-wrap gap-2 mt-2">
              {Object.keys(gist.files).map(file => (
                  <span key={file} className="px-2 py-1 bg-slate-100 dark:bg-[#233648] rounded text-xs text-slate-700 dark:text-[#c3d3e7] font-mono break-all">
                      {file}
                  </span>
              ))}
          </div>
          ...
          <main className="flex flex-col gap-6 p-0 sm:p-6">
            ...
            <div className="flex flex-wrap justify-between gap-3 px-4 sm:px-0">
              <div className="flex min-w-72 flex-col gap-3">
                <p className="text-slate-900 dark:text-white text-3xl sm:text-4xl font-black leading-tight tracking-[-0.033em]">User Dashboard</p>
                <p className="text-slate-500 dark:text-[#92adc9] text-base font-normal leading-normal">Deep dive into public user data.</p>
              </div>
            </div>
            ...
            <form onSubmit={handleSearchSubmit} className="flex flex-col sm:flex-row gap-4 items-end px-4 sm:px-0">
              ...
              <button disabled={isLoading} type="submit" className="w-full sm:w-auto flex max-w-[480px] cursor-pointer items-center justify-center overflow-hidden rounded-lg h-12 bg-primary text-white gap-2 text-sm font-bold leading-normal tracking-[0.015em] min-w-0 px-6 hover:bg-primary/90 transition-colors">
                <span className="material-symbols-outlined filled" style={{ fontSize: '20px' }}>search</span>
            ...
            <div className="flex flex-col gap-6 pt-2 sm:pt-6">
              <div className="rounded-xl border border-slate-200 dark:border-[#233648] bg-slate-50 dark:bg-[#192430] p-4 sm:p-6 mx-4 sm:mx-0">
                <div className="flex flex-col sm:flex-row items-start gap-4 sm:gap-6">
                  <img alt="User Avatar" className="h-20 w-20 sm:h-24 sm:w-24 rounded-full border-4 border-white dark:border-background-dark" src={user.avatar_url} />
                  <div className="flex flex-col gap-3 w-full">
                    <div className="flex flex-col sm:flex-row justify-between items-start">
                        <div>
                            <h3 className="text-xl sm:text-2xl font-bold text-slate-900 dark:text-white break-all">{user.login}</h3>
                            <a href={user.html_url} target="_blank" rel="noreferrer" className="text-primary hover:underline">@{user.login}</a>
                    ...
                    <div className="flex flex-wrap gap-x-6 gap-y-3 text-sm text-slate-600 dark:text-[#92adc9] mt-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span><strong className="text-slate-800 dark:text-white">{user.public_repos}</strong> Public Repos</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <span className="material-symbols-outlined text-lg">calendar_today</span>
                        <span>Joined <strong className="text-slate-800 dark:text-white">{moment(user.created_at).format('MMM DD, YYYY')}</strong></span>
                      </div>
                      <div className="flex items-center gap-2">
                        <span className="material-symbols-outlined text-lg">update</span>
                        <span>Updated <strong className="text-slate-800 dark:text-white">{moment(user.updated_at).format('MMM DD, YYYY')}</strong></span>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className="mx-4 sm:mx-0">
                <nav className="flex flex-wrap gap-2" aria-label="Tabs">
                  {[
                      ...
                      { id: 'received', label: 'Received Events', icon: 'move_to_inbox' },
                      { id: 'subscriptions', label: 'Subscriptions', icon: 'visibility' },
                      { id: 'gists', label: user.public_gists + ' Gists', icon: 'code' },
                  ].map((tab) => (
                    <button
                      key={tab.id}
              ...
              <div className="min-h-[400px] px-4 sm:px-0">
                {activeTab === 'activity' && (
                    <GenericDataTab<GitHubEvent>
                        ...
                        filterOptions={['PushEvent', 'WatchEvent', 'ForkEvent', 'CreateEvent', 'PullRequestEvent', 'IssuesEvent', 'IssueCommentEvent']}
                    />
                )}
