...
const EventDetailScreen: React.FC<EventDetailScreenProps> = ({ event, onBack }) => {
  ...
  const tabs = [
    ...
    {
      id: 'commits',
      label: `Commits (${event.payload.commits ? event.payload.commits.length : 0})`
    },
    ...
    <main className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="flex flex-wrap gap-2 mb-6">
        <button
          ...
          className="text-neutral-400 hover:text-primary text-base font-medium leading-normal transition-colors"
        >
          Home
        </button>
            ...
            <div className="grid grid-cols-[150px_1fr] gap-x-6">
              ...
              <div className="col-span-2 grid grid-cols-subgrid border-t border-t-slate-200 dark:border-t-white/10 py-4">
                ...
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="text-primary hover:underline text-sm font-normal leading-normal"
                >
                  {event.repo.name}
                </a>
              </div>
                ...
                <div className="grid grid-cols-[150px_1fr] gap-x-6 min-w-[300px]">
                  {Object.entries(event.payload).map(([key, value]) => {
                    ...
                    return (
                      <div
                        key={key}
                        className="col-span-2 grid grid-cols-subgrid py-3 border-b border-slate-200 dark:border-white/5 last:border-0"
                      >
                        <p className="text-slate-500 dark:text-neutral-400 text-sm font-normal leading-normal capitalize">
                          {key.replace(/_/g, ' ')}
                        </p>
                        <p className="text-slate-900 dark:text-white text-sm font-normal leading-normal font-mono break-all">
                          {String(value)}
                        </p>
                      </div>
                    );
                  })}
                  {Object.keys(event.payload).filter((k) => k !== 'commits' && typeof event.payload[k] !== 'object')
                    .length === 0 && (
                    <p className="text-slate-500 dark:text-neutral-400 italic">
                      No simple payload properties to display.
                    </p>
                  )}
                </div>
              )}
                ...
                <div className="flex flex-col gap-4">
                  {event.payload.commits && event.payload.commits.length > 0 ? (
                    event.payload.commits.map((commit: any) => (
                      <div
                        key={commit.sha}
                        className="p-4 rounded-lg bg-slate-200 dark:bg-white/10 flex flex-col gap-2"
                      >
